...
def plot_top_5_trajectories(ped_trajectories, data_dir):
    ...
    moving_trajectories = [t for t in ped_trajectories if t['displacement'] > 30.0]
    
    if len(moving_trajectories) >= 5:
        trajectories_to_plot = moving_trajectories
        print("\nFiltered out stationary agents (displacement < 30px) for clearer visualization.")
    else:
    ...
    for ext in ['*.png', '*.jpg', '*.jpeg']:
        found_maps = glob.glob(os.path.join(data_dir, ext))
        if found_maps:
            map_img_path = found_maps[0] 
            break
    ...
    for idx, traj in enumerate(top_5):
        ...
        if map_img_path:
            ...
            plt.ylim(cy + window, cy - window) # Inverted Y for image
        else:
