# author: me__unpredictable (Vishal Patel)
# vishalresearch.com
# this is C-TAG: Contextual Temporal Attention Graph for Trajectory Prediction
import os
import math
import sys
import time
...
train_files = glob.glob(os.path.join(processed_train_dir, "*.pkl"))
files_exist = len(train_files) > 0

if args.reload_data or not files_exist:
    print("Processed data missing or reload requested. Generating splits...")
    # Initialize with RAW path to trigger generation. 
    # utils.py auto-detects Raw path vs Pkl path.
    _ = TrajectoryDataset(
        data_dir=args.dataset_path, 
        obs_len=args.obs_seq_len,
        pred_len=args.pred_seq_len,
        skip=1, norm_lap_matr=True, delim=args.delim, 
...
loader_train = DataLoader(
    dset_train,
    batch_size=1, # Must be 1 for variable sequence length support via collate
...
def train(epoch, model, optimizer, loader_train, metrics):
    ...
    for cnt, batch in enumerate(loader_train): 
        ...
        V_obs = V_obs.unsqueeze(0) # Fix dimensions for model
        ...
        V_pred = V_pred.squeeze(0) # Remove batch dim

        l = graph_loss(V_pred, V_tr)

        # FIX: UnboundLocalError by properly initializing loss in all branches
        if is_fst_loss:
            loss = l
            is_fst_loss = False
        else:
        ...
        if batch_count % args.batch_size == 0 or cnt == turn_point:
            ...
            print('TRAIN:', '\t Epoch:', epoch, '\t Loss:', loss_batch / (cnt + 1) * args.batch_size)
            
            is_fst_loss = True
            loss = 0 # Reset for next batch accumulation
    ...
    with torch.no_grad():
        for cnt, batch in enumerate(loader_val): 
            ...
            if batch_count % args.batch_size == 0 or cnt == turn_point:
                loss = loss / args.batch_size
                loss_batch += loss.item()
                print('VALD:', '\t Epoch:', epoch, '\t Loss:', loss_batch / (cnt + 1) * args.batch_size)
                is_fst_loss = True
                loss = 0
...
for epoch in range(args.num_epochs):
    ...
    if args.use_lrschd and scheduler is not None:
...
if not args.skip_val:
    print("Calculating final ADE/FDE...")
    # Load best model
    ...
    print(f"Final Best Model - ADE: {ade_calc}, FDE: {fde_calc}")
